...
public class APIManager: NSObject, URLSessionDelegate {
    ...
    public func getResponse(urlString: String, params: [String: Any]?, completion connectionCompletion: @escaping (_ error: Error?) -> Void) {
     ...
     */
    private func fetchResponse(urlString: String, params: [String: Any]?, methodType method: HTTPMethod, completion connectionCompletion: @escaping (_ error: Error?) -> Void) {
            ...
            session.dataTask(with: apiRequest as URLRequest, completionHandler: {(data, response, error) -> Void in
                ...
                if let httpResponse: HTTPURLResponse = response as? HTTPURLResponse {
                    if httpResponse.statusCode == 200 {
                        ResponseData.shared.fetchData(responseData: data)
                        connectionCompletion(error as NSError?)
                    } else {
                        connectionCompletion(self.handleError(statusCode: httpResponse.statusCode))
                    }
                } else {
                    print(error!)
                    ...
                    connectionCompletion(err as NSError)
                }
            }).resume()
